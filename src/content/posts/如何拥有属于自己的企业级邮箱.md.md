...
**官方文档**：https://doc.skymail.ink/guide/dashboard.html

---

> [!IMPORTANT] IMPORTANT
> 本教程严格按照官方文档使用 **Workers** 方式部署（非 Pages）。Pages 方式可能导致 Email Routing 绑定异常。
...
1. 打开 [https://github.com/maillab/cloud-mail](https://github.com/maillab/cloud-mail)
...
2. 点击 **Create application** → 选择 **Workers** → **Connect to Git**
...
```text
https://mail.example.com/api/init/your-super-long-random-jwt-secret-2026
```
...
- 点击注册
- **用户名**：`admin`
- **邮箱**：`admin@example.com`（必须与环境变量完全一致）
...
2. 同意自动添加 MX 记录
3. **Routing rules** → **Create address** → **Catch-all address**
4. Destination 选择你的 Worker

> [!TIP] TIP
> Catch-all 功能非常强大，任意前缀的邮箱都能接收，无需提前创建账号。
...
2. 创建机器人并复制 Token
3. 给机器人发消息
4. 浏览器访问 `https://api.telegram.org/bot你的Token/getUpdates` 获取 chat_id
